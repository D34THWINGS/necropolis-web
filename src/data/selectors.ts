...
import { SOUL_STORM_DEFENSE_BONUS, SOUL_STORM_LETHALITY_BONUS } from '../config/constants'
...
export const getDefense = (state: RootState) =>
  getBattlementsDefenseBonus(getBattlements(state).level) + (getIsSoulStormActive(state) ? SOUL_STORM_DEFENSE_BONUS : 0)
