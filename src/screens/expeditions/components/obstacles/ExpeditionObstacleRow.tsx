...
const obstacleRowSlot = css({
  ...
  height: '4rem',
  backgroundColor: frameColors.DARK_GREEN,
})
...
export const ExpeditionObstacleRow = ({ title, row, undeads, isActive, onToggleRow }: ExpeditionObstacleRowProps) => {
  ...
  return (
    <div key={row.id} css={obstacleRowWrapper}>
      ...
      <div css={obstacleRowBody(isActive)}>
        <div css={obstacleRowStats}>
          <div css={obstacleRowRequirements}>
        ...
        <div css={obstacleRowSlotsList}>
          {Array.from({ length: row.diceSlots })
            ...
            .map(index => {
              ...
              return (
                <div key={index} css={obstacleRowSlot}>
                  {slottedUndead && (
                    <button type="button">
                      <UndeadPortrait type={slottedUndead.type} />
                      &nbsp;
                      {getUndeadDice(slottedUndead, row.requiredTalent[0]).maxValue}
                    </button>
                  )}
                  {!slottedUndead && (
